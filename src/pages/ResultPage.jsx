...
export default function ResultPage({ reading, question, onRestart, onBack }) {
  ...
  useEffect(() => {
    ...
    const cardList = enrichedPositions
      .filter(p => p.card && p.id !== 'emphasis') // 定调牌不解读，只定调
      .map(p => ({
        position: p.label,
        positionDesc: p.desc,
        name: p.card.name_zh,
...
请按每张牌的顺序解读，紧密结合客户的具体问题展开。每段150-250字，温暖真诚。每一张牌的解答都要挖到它反映的核心信念，不要只在最后总结一句。`
      ...
      const empEl = reading.cards['emphasis']?.card?.element || null
      const empLine = empEl
        ? `\n\n定调牌：你抽到的这张定调牌，证明了影响你最深的是「${ELEMENT_INFO[empEl].name}元素 · ${ELEMENT_INFO[empEl].belief}」。这场解读、尤其是最后的「主信念」，请更看重这个方向。`
        : ''
      systemPrompt = `你是 DO!Hand 工作室的塔罗解读师。解读风格：温暖、有洞察力、真诚不玄乎。
...
- 最后加一段「主信念」：点出四条信念里哪一条是根，另外三条其实在为它服务（2-3 句）。${empLine}
